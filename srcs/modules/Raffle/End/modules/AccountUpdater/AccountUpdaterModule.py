...
class AccountUpdaterModule():
    ...
    def __init__(self) -> None:
        ...
        self.subModuleName = "AccountUpdater"
        ...
        while runningLoop:
            if (self.threads > 1):
                with ThreadPoolExecutor(max_workers=self.threads) as executor:
                    ...
                    for task in as_completed(tasks):
                        ...
                        setTitle(f"SourceRaffles - End [{ProfileName}] | Success: {str(successTask)} - Failed {str(failedTask)} - Remaining {str(remaining)}")
            else:
                printSeparator()
                for index, profile in enumerate(Profiles):
            ...
            setTitle(f"SourceRaffles - End [{ProfileName}] | Ended! Tasks: {str(successTask)}/{str(failedTask)}")
