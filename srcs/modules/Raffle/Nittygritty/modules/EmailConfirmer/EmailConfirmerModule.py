...
class EmailConfirmerModule():
    ...
    def __init__(self) -> None:
        ...
        while runningLoop:
                ...
                with ThreadPoolExecutor(max_workers=self.threads) as executor:
                    for index, profile in enumerate(Profiles):
                        ...
                        if (profile['status'] == "PENDING" or profile['status'] == "FAILED"):
                            tasks.append(executor.submit(EmailConfirmerTask, index, profilesNumber, profile))
                    ...
                    for task in as_completed(tasks):
                        ...
                        setTitle(f"SourceRaffles - Nittygritty [{ProfileName}] | Success: {str(successTask)} - Failed {str(failedTask)} - Remaining {str(remaining)}")
            else:
                printSeparator()
                for index, profile in enumerate(Profiles):

                    if (profile['status'] == "PENDING" or profile['status'] == "FAILED"):
                        TaskResult = EmailConfirmerTask(index, profilesNumber, profile)
            ...
            setTitle(f"SourceRaffles - Nittygritty [{ProfileName}] | Ended! Tasks: {str(successTask)}/{str(failedTask)}")
