...
class EnterRaffleModule:
    ...
    def __init__(self) -> None:
        ...
        if (questionary.confirm(f"AutoConfirm your entries?").ask()):
            self.raffle['AutoConfirmer'] = {
                "status": True,
                "delay": 15
            }
        else:
        ...
        while runningLoop:
            if self.threads > 1:
                with ThreadPoolExecutor(max_workers=self.threads) as executor:
                    ...
                    for task in as_completed(tasks):
                        ...
                        setTitle(
                            f"SourceRaffles - BSTN [{ProfileName}] | Success: {str(successTask)} - Failed {str(failedTask)} - Remaining {str(remaining)}"
                        )
            else:
                printSeparator()
                for index, profile in enumerate(Profiles):
            ...
            setTitle(
                f"SourceRaffles - BSTN [{ProfileName}] | Ended! Tasks: {str(successTask)}/{str(failedTask)}"
            )
