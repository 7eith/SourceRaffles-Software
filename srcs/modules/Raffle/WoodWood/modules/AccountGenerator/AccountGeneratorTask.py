    ...
    def solveCloudflare(self):
        ...
        try:
            response = self.session.get(
                "https://www.woodwood.com/en/auth/view?type=register"
            )

            Logger.debug(f"{self.logIdentifier} Received {response.status_code} from Woodwood [SolveCloudflare]")
            ...
            Logger.debug(f"{self.logIdentifier} Error while solving CloudFlare on Woodwood: {str(error)}")
            ...
            Logger.debug(f"{self.logIdentifier} ConnectionError while solving CloudFlare on Woodwood: {str(error)}")
    ...
    def createAccount(self):
        ...
        if (self.captchaToken == None):
            ...
            captcha = CaptchaHandler().handleRecaptcha(
                '6Lc1_6wZAAAAAPHAgQR1VOEkQjVo0OpZT41rrjxa',
                "https://www.woodwood.com/en/auth/view?action=register",
                invisible=1
            )
        ...
        try:
            response = self.session.post("https://www.woodwood.com/en/auth/submit", data=payload)

            Logger.debug(f"{self.logIdentifier} Received {response.status_code} from Woodwood [RegisterAccounts]")
            Logger.debug(f"{self.logIdentifier} Received {response.text} from Woodwood [RegisterAccounts]")
