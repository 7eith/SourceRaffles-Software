...
class AccountUpdaterTask():
    ...
    def __init__(self, index, taskNumber, profile) -> None:
        ...
        self.adyenKey = "10001|"
        self.liveKey = "live_VDZDBWVCHFHAHARVZIFF3UTWVQIKTIOI"
        self.paymentUrl = 'https://checkoutshopper-live.adyen.com/checkoutshopper/v68/paybylink/payments&d=PLC57C04E1EF0D752F'
    ...
    def updateAccount(self):
            ...
            if response.status_code != 200:
                ...
                try:
                    ...
                    if message == "User does not exist.":
                        Logger.info(f"{self.logIdentifier} Account never logged on the SNS app, creating session...")
        ...
        liveToken = "live_7G47GNRX4BCCJEX7VCJY6MHE7UVIUGIZ"
        ...
        if response.status_code == 200:
            try:
                ...
                if errorCode == "Refused":
                    Logger.error(f"{self.logIdentifier} Card refused by SNS")
                    return -1
            except KeyError:
    ...
    def submit_payment(self):
        adyen_encryptor = encrypt.Encryptor(self.adyenKey)
        while True:
            try:
                cc_data = {
                    ...
                    "paymentMethod":
                    {
                        ...
                        "brand": bin_lookup.get_card_brand(self.session, adyen_encryptor.encrypt_card_data(bin_value=self.profile["cc_number"][:6]), supported_brands= ["amex", "diners", "jcb", "mc", "visa"], live_key=self.liveKey)
                    },
                    "browserInfo":
                    {
                        "acceptHeader": "*/*",
            ...
            try:
                response = self.session.post(url=self.paymentUrl, json=cc_data)
            except Exception as e:
    ...
    def threeDS(self, json):
        ...
        try:
            ...
            def redirect_hook(driver):
                Logger.info("Please confirm that you solved 3ds : Yes/No")
                answer = input("Answer : ")
                if answer == "Yes":
                    return True
                else:
            ...
            threeds_result = handle_3ds(
                payment_action=payment_action,
                session=self.session,
                live_token=self.liveKey,
                return_url=self.paymentUrl,
                redirect_hook=redirect_hook)

            if threeds_result is not None:
                Logger.debug(threeds_result)
