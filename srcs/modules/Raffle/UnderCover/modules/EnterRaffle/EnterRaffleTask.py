...
class EnterRaffleTask:
    ...
    def initializeSession(self):
        ...
        try:
            response = self.session.get("https://store.undercoverism.com/login")
    ...
    def login(self):
        ...
        try:
            response = self.session.post('https://store.undercoverism.com/login', data=data)
                
            if ("error-messages" in response.text):
                Logger.error(f"{self.logIdentifier} This account doesn't exist on UnderCover! ")
                return (FAILED)
    ...
    def enterRaffle(self):
        data = {
            ...
            'color_code': self.raffle['metadata']['colorCode'],
            'size_code': self.SizeDict['id'],
            'pod': '1'
        }
        ...
        try:
            ...
            response = self.session.post(
                'https://store.undercoverism.com/apis/lottery/sale/entry.json', 
                headers={
                    ...
                    'Referer': f"https://store.undercoverism.com/specialitem/item/detail/{self.raffle['metadata']['detailCode']}/{self.raffle['metadata']['colorCode']}",
                },
                data=data
            )
            ...
            if (response.json()['status'] == "success"):
                ...
                if (response.json()['errors'] == "This product has already been entered in the lottery."):
