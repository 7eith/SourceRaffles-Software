...
class EnterRaffleModule:
    ...
    def PromptRaffle(self):
        ...
        #     f"{apiURL}/raffles", json={"moduleName": self.moduleName}
        # )

        raffles = [
            {
                "name": "Yeezy - Foam Runner [Ochre]",
                "product": "Yeezy - Foam Runner Ochre",
                "url": "https://www.impact-premium.com/cf/raffle-1",
                "image": "https://www.impact-premium.com/img/cms/RAFFLE%20CONCOURS%20IMAGE%20TEMPLATE.jpg",
                "active": True,
                "metadata": [
                    {
                        "entryURL": "https://www.impact-premium.com/cf/raffle-1",
                        "sizes": [
                            "37  EU - 4 US",
                            "38 EU - 5 US",
                            "39  EU - 6 US",
    ...
    def __init__(self) -> None:
        ...
        while runningLoop:
            if self.threads > 1:
                with ThreadPoolExecutor(max_workers=self.threads) as executor:
                    ...
                    for task in as_completed(tasks):
                        ...
                        setTitle(
                            f"SourceRaffles - Impact [{ProfileName}] | Success: {str(successTask)} - Failed {str(failedTask)} - Remaining {str(remaining)}"
                        )
            else:
                printSeparator()
                for index, profile in enumerate(Profiles):
            ...
            setTitle(
                f"SourceRaffles - Impact [{ProfileName}] | Ended! Tasks: {str(successTask)}/{str(failedTask)}"
            )
