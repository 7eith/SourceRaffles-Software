...
from modules.Raffle.Typeforms.modules.EnterRaffle.EnterRaffleTask import EnterRaffleTask
...
class EnterRaffleModule:
    ...
    def PromptRaffle(self):
        ...
        raffleUrl = input("Enter the url of the raffle to enter : ")
        return raffleUrl
    ...
    def __init__(self) -> None:
        ...
        """ Load Profiles & Proxy """
        ProfilesManager(self.moduleName, self.subModuleName, checkTasks=False).loadProfiles()
        ...
        while runningLoop:
            if self.threads > 1:
                with ThreadPoolExecutor(max_workers=self.threads) as executor:
                    ...
                    for task in as_completed(tasks):
                        ...
                        setTitle(
                            f"SourceRaffles - Typeforms [{ProfileName}] | Success: {str(successTask)} - Failed {str(failedTask)} - Remaining {str(remaining)}"
                        )
            else:
                printSeparator()
                for index, profile in enumerate(Profiles):
            ...
            setTitle(
                f"SourceRaffles - Typeforms [{ProfileName}] | Ended! Tasks: {str(successTask)}/{str(failedTask)}"
            )
