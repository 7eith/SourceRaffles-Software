...
class AccountConfirmerTask:
    ...
    def scrapeEmails(self):
        try:
            emailConfirm = self.profile["email"]
            passwordConfirm = self.profile["password"]
            ...
            with MailBox(imap_server).login(emailConfirm, passwordConfirm, 'INBOX') as mailbox:
                for msg in mailbox.fetch(A(seen=False, from_="ec@sacai.jp")):
                    ...
                    for url in urls:
                        if "access_key" in url and "sacai" in url:
                            Logger.info("URL found : {}".format(url))
                            self.list_urls.append(url)
            if len(self.list_urls) == 0:
                Logger.error("No new link found in your mailbox")
                return -1
            else:
                Logger.info("{} link(s) found, starting confirmation.".format(len(self.list_urls)))
                return self.confirmLinks()
        except Exception as e:
            Logger.error("Error while scraping urls : {}".format(str(e)))
            return -1

    def confirmLinks(self):
        for link in self.list_urls:
            Logger.info("Confirming account for link : {}".format(link))
            try:
                r = self.session.get(link)
                if r.status_code == 200:
                    Logger.info("Accessed main page, confirming password...")
                    ...
                    data = {
                        ...
                        'password': self.profile["password_account"],
                        'end': 'Register'
                    }

                    response = self.session.post('https://store.sacai.jp/signup/step04', headers=headers, data=data)
                    if response.status_code == 200:
                        Logger.success("Account successfully confirmed !")
                        return 1

                else:
                    Logger.error("Error while confirming account : {}".format(r.status_code))
                    return -1
            except Exception as e:
                Logger.error("Error while confirming account : {}".format(str(e)))
        ...
        try:
            ...
            if self.profile["link"] == "":
                ...
                if statusScrapeUrl == 1:
                    Logger.success("Links scraped successfully !")

                    confirmAccounts = self.confirmLinks()

                    if confirmAccounts == 1:
                        Logger.success("Accounts confirmed !")
                        self.success = True
                        self.profile["status"] = "SUCCESS"
