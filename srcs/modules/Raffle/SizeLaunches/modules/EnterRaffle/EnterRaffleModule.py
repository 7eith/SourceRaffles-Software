...
from modules.Raffle.SizeLaunches.modules.EnterRaffle.EnterRaffleTask import EnterRaffleTask
...
class EnterRaffleModule:
    ...
    def __init__(self) -> None:
        ...
        while runningLoop:
            if self.threads > 1:
                with ThreadPoolExecutor(max_workers=self.threads) as executor:
                    ...
                    for task in as_completed(tasks):
                        ...
                        setTitle(
                            f"SourceRaffles - SizeLaunches [{ProfileName}] | Success: {str(successTask)} - Failed {str(failedTask)} - Remaining {str(remaining)}"
                        )
            else:
                printSeparator()
                for index, profile in enumerate(Profiles):
            ...
            setTitle(
                f"SourceRaffles - SizeLaunches [{ProfileName}] | Ended! Tasks: {str(successTask)}/{str(failedTask)}"
            )
